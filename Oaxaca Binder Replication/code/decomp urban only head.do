...
global demographics gender age agesq hhsize casado hh_size_02 hh_size_311  hh_size_1217  hh_size_1859  hh_size_60   //10
global labor i.relab // 5
global educ  i.edattain i.hh_max_edattain // 8
*global head i.hh_head_edattain hh_fhead //2 
*global utilities hh_piped_to_prem hh_sewage hh_toilet hh_elect //4
...
qui: eststo: xi: oaxaca lhh_labinc_pc_ppp11 $demographics $labor $educ  i.year [aw=pondera], by(noleading1) vce(robust) weight(1) relax detail(demographics: $demographics, education : $educ, labor: $labor, year: i.year) 
		...
		estadd local Nobs = nonb   // number of observations	
			...
			preserve
			matsave decomp_detail_`period', p("$user/_OaxacaBlinder_replication/outputs/`code'/metro/urban") dropall replace
			restore 
...
// bottom 40% of adults heads (as opposed to all adults)
...
esttab, r2 star(* 0.10 ** 0.05 *** 0.01) p(3) b(3) label keep(group_1 group_2 difference explained unexplained)
	...
	set more off
	#delimit;
	esttab using "$user/_OaxacaBlinder_replication/outputs/`code'/metro/oaxaca_`code'_`period'_h.tex",  mlabels(,none) 
	star(* 0.10 ** 0.05 *** 0.01) b(3) se(3) r2  noisily bookt label replace notype keep(group_1 group_2 difference explained unexplained)
...
use "$user/_OaxacaBlinder_replication/outputs/`code'/metro/urban/clean_eform_1", clear
		gen group = "urban" 
		gen period = "1" 
...
use "$user/_OaxacaBlinder_replication/outputs/`code'/metro/b40/tmp_all"
	gen group = "bottom40" 
