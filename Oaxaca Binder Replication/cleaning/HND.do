global user "C:\Users\wb486315\OneDrive - WBG/Inequality/LAC"
   ...
   local files : dir "$user/_OaxacaBlinder_replication/SEDLAC/HND/" files"*.dta", respectcase 
    foreach file in `files' {
        append using "$user/_OaxacaBlinder_replication/SEDLAC/HND/`file'", force
	}

tab year
drop p1* P1* p2* P2* p3* P3* p4* P4* p5* cp5* p6* P6*
drop p7* P7* p8* P8* p9* P9* fep* 

gen countrycode = upper(pais)
rename encuesta survey  

tab region_est1 // region
tab region_est2 // department
...
sum ilpc // per capita labor income 
...
tab year, sum(ipc_sedlac) // for time comparison
	...
	gen leading1 = 0
		replace leading1 = 1 if dept == 8 & urbano == 1 //  Tegucigalpa (urban Francisco Morazan?)
...
tab year, sum(lp_moderada)

** correct income before deflating properly

foreach inc in ila ilf ilpc ii ipcf_sr itf{
replace   `inc'   =   `inc' * p_reg if urbano == 0   /*  adjusting  */

	}

	
gen   pl_ll =  lp_moderada   if   dept == 8 & urbano == 1     /*  poverty line   */
...
// 	logs
	...
	gen lind_totinc_11 = log(ind_totinc_ppp11)	
		...
		tab dept [w=pondera_i] if cohi == 1 & year == 2019, sum(hh_totinc_pc_ppp11)

	
** Bottom 40%

	foreach year in 2001 2002 2004 2005 2006 2007 2008 2009 2011 2012 2013 2014 2015 2016 2017 2018 2019  {
...
egen hh_elect = mean(elect), by(id year)  
	...
	gen period = .
	replace period = 1 if year >= 2004 & year <= 2006
	*replace period = 2 if year >= 2006 & year <= 2008
	*replace period = 3 if year >= 2009 & year <= 2013
	...
	gen adm1 = region_est22 
		...
		keep adm1 id com urbano region* jefe dept year period leading* noleading* lind* lhh* ind_* hh_* ila ilf ilpc ii ipcf_sr itf p_reg b40* skilled* cohh cohi pondera* gender age agesq hhsize casado hh_size_02 hh_size_02_sq hh_size_311 hh_size_311_sq hh_size_1217 hh_size_1217_sq hh_size_1859 hh_size_1859_sq hh_size_60 hh_size_60_sq empstat* relab* edattain* hh_max_edattain* hh_piped_to_prem hh_elect hh_sewage hh_toilet lp_moderada adj_urb adm1 p_reg //def 

		
save "$user/_OaxacaBlinder_replication/inputs/HND_SEDLACsmall.dta", replace
