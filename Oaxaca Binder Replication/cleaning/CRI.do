...
// Download data
 ...
 dlw, coun(CRI) y(2010 2011 2012 2013 2014 2015 2016 2017 2018 2019 2020) t(SEDLAC-03) mod(ALL)  sur(ENAHO) 
 save "$user/_OaxacaBlinder_replication/SEDLAC/CRI/CRI_SEDLAC1020.dta"
 
 dlw, coun(CRI) y(1989 1990 1991 1992 1993 1994 1995 1996 1997 1998 1999 2000 2001 2002 2003 2004 2005 2006 2007 2008 2009 2010 2011 2012 2013 2014 2015 2016 2017 2018 2019 2020) t(SEDLAC-03) mod(ALL)  sur(EHPM) 
 save "$user/_OaxacaBlinder_replication/SEDLAC/CRI/CRI_SEDLAC8909.dta"
*/
...
use "$user/_OaxacaBlinder_replication/SEDLAC/CRI/CRI_SEDLAC1020.dta", clear
	append using "$user/_OaxacaBlinder_replication/SEDLAC/CRI/CRI_SEDLAC8909.dta"
	...
	tab region_est1 urbano // San jose is in central 
	
	*keep year region_est2 lp urbano
...
gen region = real(region_est11)   // state only available for early periods 
						
split region_est2, parse(" - ")
gen regionSJ = real(region_est21)   // state only available for early periods 
...
tab year, sum(ipc_sedlac) // for time comparison
	...
	gen leading1 = 0
	replace leading1 = 1 if region == 1 & urbano == 1 // San Jose (Urban Central)
...
gen   pl_ll =  pl_mean   if   region == 1 & urbano == 1     /*  poverty line from urban Central   */
...
foreach inc in ila ilf ilpc ii ipcf_sr itf{
replace   `inc'   =   `inc'  /  adj_urb   if year >= 2010   /*  adjusting when survey data exists  */
...
tab region_est11 [w=pondera_i] if cohh == 1 & year == 2020, sum(hh_totinc_pc_ppp11)
...
egen hh_elect = mean(elect), by(id year)  
	...
	*replace period = 1 if year >= 2010 & year <= 2013
	*replace period = 2 if year >= 2014 & year <= 2016
	replace period = 3 if year >= 2008 & year <= 2009
	
	*replace period = 4 if year == 2020
	*gen p2020 = 1 if year == 2020 
	

	

	encode region_est12, gen(adm)

	label define adm 4 "Huetar Atlántica", modify
	label define adm 5 "Huetar Atlántica", modify
	label define adm 8 "Pacifico Central", modify
	...
	keep id com urbano region* year leading* noleading* lind* lhh* ind_* hh_* ila ilf ilpc ii ipcf_sr itf def p_reg b40* skilled* cohh cohi pondera* gender age agesq hhsize jefe casado hh_size_02 hh_size_02_sq hh_size_311 hh_size_311_sq hh_size_1217 hh_size_1217_sq hh_size_1859 hh_size_1859_sq hh_size_60 hh_size_60_sq empstat* relab* edattain* hh_max_edattain* hh_piped_to_prem hh_elect hh_sewage hh_toilet lp_moderada adm1 jefe
...
save "$user/_OaxacaBlinder_replication/inputs/CRI_SEDLACsmall.dta", replace
