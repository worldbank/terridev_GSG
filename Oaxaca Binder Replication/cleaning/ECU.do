...
* dlw, coun(ECU) y(2003 2004 2005 2006 2007 2008 2009 2010 2011 2012 2013 2014 2015 2016 2017 2018 2019 2020) t(SEDLAC-03) mod(ALL)  sur(ENEMDU) 
* save "$user/_OaxacaBlinder_replication/SEDLAC/ECU/ECU_SEDLAC", replace

use "$user/_OaxacaBlinder_replication/SEDLAC/ECU/ECU_SEDLAC.dta", clear
drop if year == 2020
...
*tab region_est2 // province

label list prov 
decode prov, gen(adm1)
...
tab year, sum(ipc_sedlac) // for time comparison
	...
	gen leading1 = 0
		replace leading1 = 1 if prov == 17 & urbano == 1 // only  Pichincha urban (quito) 
...
** delfating - pl_mod is only national in the survey --> CEPAL 
...
foreach inc in ila ilf ilpc ii ipcf_sr itf{
	replace `inc' = `inc'*p_reg if urbano == 0 // correcting 
	replace   `inc'   =   `inc'  /  def if urbano == 0  /*  adjusting  */
...
// 	logs
	...
	gen lind_totinc_11 = log(ind_totinc_ppp11)	
		...
		*tab prov [w=pondera_i] if cohi == 1 & year == 2020, sum(hh_totinc_pc_ppp11)
	
	
** Bottom 40%

	foreach year in 2003 2004 2005 2006 2007 2008 2009 2011 2012 2013 2014 2015 2016 2017 2018 2019 2020 {
...
egen hh_elect = mean(elect), by(id year)  
	...
	gen period = .
	replace period = 1 if year >= 2003 & year <= 2004
	...
	*replace period = 3 if year >= 2012 & year <= 2015
	...
	tab year period
		...
		keep id com urbano region* prov year period leading* noleading* lind* lhh* ind_* hh_* ila ilf ilpc ii ipcf_sr itf p_reg b40* skilled* cohh cohi pondera* gender age agesq hhsize casado hh_size_02 hh_size_02_sq hh_size_311 hh_size_311_sq hh_size_1217 hh_size_1217_sq hh_size_1859 hh_size_1859_sq hh_size_60 hh_size_60_sq empstat* relab* edattain* jefe hh_max_edattain* hh_piped_to_prem hh_elect hh_sewage hh_toilet lp_moderada  def  adm1  //adj_urb
...
save "$user/_OaxacaBlinder_replication/inputs/ECU_SEDLACsmall.dta", replace
