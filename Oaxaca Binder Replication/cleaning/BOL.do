...
/*
dlw, coun(BOL) y(2001 2002 2005 2006 2007 2008 2009  ) t(SEDLAC-03) mod(ALL)  sur(EH) 
save "$user/_OaxacaBlinder_replication/SEDLAC/BOL/BOL_SEDLAC0109.dta", replace

dlw, coun(BOL) y(2010 2011 2012 2013 2014 2015) t(SEDLAC-03) mod(ALL)  sur(EH) 
save "$user/_OaxacaBlinder_replication/SEDLAC/BOL/BOL_SEDLAC1115.dta", replace

dlw, coun(BOL) y(2016 2017 2018 2019 2020) t(SEDLAC-03) mod(ALL)  sur(EH) 
save "$user/_OaxacaBlinder_replication/SEDLAC/BOL/BOL_SEDLAC1620.dta", replace
...
use "$user/_OaxacaBlinder_replication/SEDLAC/BOL/BOL_SEDLAC0109.dta", clear

drop s1_* s2_* s3_* s4_* s5_* s6_* s7_* s8_* s9_*
 
	append using "$user/_OaxacaBlinder_replication/SEDLAC/BOL/BOL_SEDLAC1620", force
	
drop s1* s2* s3* s4* s5* s6* s7* s8* s9*
 	
	append using "$user/_OaxacaBlinder_replication/SEDLAC/BOL/BOL_SEDLAC1115", force 

drop s01* s02* s03* s04* s05* s06* s07* s08* s09*
...
gen   pl_ll =  lp_moderada   if   dept == 7  & urbano == 1     /*  poverty line from urban SC   */
...
replace   `inc'   =    `inc'  * p_reg if urbano == 0   /*   remove sedlac adjustment  (no neded reely in this case since p_reg=1    */
...
// 	logs
	...
	gen lind_totinc_11 = log(ind_totinc_ppp11)	
		...
		tab region_est22 [w=pondera_i] if cohi == 1 & year == 2020, sum(hh_totinc_pc_ppp11)

	gen leading1 = 0
		replace leading1 = 1 if dept == 7 & urbano == 1 // only urban Santa Cruz 
		
	tab dept	
	
	gen leading2 = 0
	replace leading2 = 1 if dept == 2 & urbano == 1 // La Paz 
...
** Bottom 40%
	...
	foreach year in 2001 2002 2005 2006 2007 2008 2009 2011 2012 2013 2014 2015 2016 2017 2018 2019 2020{
...
egen hh_elect = mean(elect), by(id year)  
	...
	*replace period = 2 if year >= 2009 & year <= 2014
	*replace period = 3 if year >= 2015
	...
	*gen p2020 = 1 if year == 2020
	...
	gen adm1 = region_est22
		...
		keep id com urbano region* dept year period leading* noleading* lind* lhh* ind_* hh_* ila ilf ilpc ii ipcf_sr itf p_reg b40* skilled* cohh cohi pondera* gender age agesq hhsize casado hh_size_02 hh_size_02_sq hh_size_311 hh_size_311_sq hh_size_1217 hh_size_1217_sq hh_size_1859 jefe hh_size_1859_sq hh_size_60 hh_size_60_sq empstat* relab* edattain* hh_max_edattain* hh_piped_to_prem hh_elect hh_sewage hh_toilet lp_moderada adj_urb  adm1 jefe
			
	tab year leading1 [w=pondera]
	tab year leading1 [w=pondera] if lind_labinc_ppp11 != ., row nofreq
		
save "$user/_OaxacaBlinder_replication/inputs/BOL_SEDLACsmall.dta", replace
