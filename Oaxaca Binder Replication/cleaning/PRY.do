...
* dlw, coun(PRY) y(2002 2003 2004 2005 2006 2007 2008 2009 2010 2011 2012 2013 2014 2015 2016 2017 2018 2019 2020) t(SEDLAC-03) mod(ALL)  sur(EPH) 
* save "$user/_OaxacaBlinder_replication/SEDLAC/PRY/PRY_SEDLAC.dta"

use "$user/_OaxacaBlinder_replication/SEDLAC/PRY/PRY_SEDLAC.dta", clear
drop if year == 2020
count 
tab year
...
tab year, sum(ipc_sedlac) // for time comparison
	...
	gen leading1 = 0
		replace leading1 = 1 if dept == 0 // only Asuncion 
...
** Demographics
*Labor characteristics (self-employment, wage earner, unpaid worker, unemployed/Inactive) 
...
egen hh_elect = mean(elect), by(id year)  
	...
	*replace period = 2 if year >= 2008 & year <= 2013
	*replace period = 3 if year >= 2014 & year <= 2019
	...
	gen adm1 = region_est22 
		...
		keep id com urbano region* dept year period leading* noleading* lind* lhh* ind_* hh_* ila ilf ilpc ii ipcf_sr itf p_reg b40* skilled* cohh cohi pondera* gender age agesq hhsize casado hh_size_02 hh_size_02_sq hh_size_311 hh_size_311_sq hh_size_1217 hh_size_1217_sq hh_size_1859 hh_size_1859_sq hh_size_60 hh_size_60_sq empstat* relab* edattain* hh_max_edattain* hh_piped_to_prem hh_elect hh_sewage hh_toilet lp_moderada adj_urb def adm1 jefe 

			tab year leading1 [w=pondera] if lind_labinc_ppp11 != ., row nofreq
			
save "$user/_OaxacaBlinder_replication/inputs/PRY_SEDLACsmall.dta", replace
