...
dir_out<- "./Data/Benchmarking/SD_Profiles/Output/"              #In this directory create with separate folders for income group - ej. lmics, linc, umic
getwd()
```
#Dataset creation
```{r}
...
country_estimates_global <- read.csv(paste0(dir_int, "World_SD_values1.csv"))
...
#Regional group
regional_group<- "Middle East & North Africa"  ### HERE CHOOSE REGIONAL GROUP - FOR BARPLOT OF REGIONAL COMPARISON  ONLY select between: "Sub-Saharan Africa", "South Asia", "Latin America & Caribbean", "Middle East & North Africa", "Europe & Central Asia", "East Asia & Pacific" - NAME IN "")
...
### Version indicator
#choose 0 or 1. 
#v= 0 Refers to a set of indicators selected and vetted by the Chieft Economist Office -Sustainable Development (work lead by Jason Daniel Russ)
# v=1 refers to a set of indicators selected by the MENA - Sustainable Development team foscusing on Egypt results 
#Each versio would show a different set of indicators
version <- 1 # or 0
```
...
country_all2<-country_all2%>% select(country, iso3, income, region, Resiliance_v1, Resiliance_v2, Inclusion_v1, Inclusion_v2, Sustainability_v1, Sustainability_v2, Efficiency_v1, Efficiency_v2)

#Tranform to long format for graph
country_long_complete <- reshape2::melt(country_all2,
        # ID variables - all the variables to keep but not split apart on
    id.vars=c("country", "iso3", "income", "region"),
    variable.name="pilar",
    value.name="percentil"
)

#Get the median value for income group & transform to ling
median_income<- country_all2 %>%
  group_by(income)%>%
    dplyr::summarise(across(where(is.numeric), function(x){ median(x, na.rm=TRUE)}))

median_income <- reshape2::melt(median_income,
        # ID variables - all the variables to keep but not split apart on
    id.vars=c("income"),
    variable.name="pilar",
...
#Join data 
country_long_complete<-left_join(country_long_complete, median_income, by=c("pilar", "income"))
```

#Function for PERCENTILE RANKING BAR GRAPH - Select regional_group in advance 
#From this point version selection between version 1 or 0, affects the outcomes 
```{r}
for(i in country){
...
country_long<-country_long%>% filter(str_detect(pilar, "v2"))
}
...
country_long<-country_long%>%
  dplyr::filter(region==regional_group)%>%
  dplyr::filter(!is.na(percentil))%>%
  ungroup() %>%
  arrange(pilar, (desc(percentil)))%>%
  mutate(r = row_number())%>%
  mutate(pilar_name = str_replace(pilar, "_v1|_v2", ""))

country_long$pilar_name<-str_replace(country_long$pilar_name, "Resiliance", "Resilience")
country_long$pilar_name_f<- factor(country_long$pilar_name, levels= c("Resilience", "Inclusion", "Sustainability", "Efficiency"))

#Once filtered by region, get regional median graph  
country_long<-country_long%>%
  group_by(pilar)%>%
...
reg<-  ggplot(country_long, aes(x = r, y = percentil*100, fill = pilar_name_f, color = as.factor(country_ind), group=income), alpha=0.7) + 
  geom_col(size=1.2)+
  ...
  title = "Percentil Ranking, against regional pairs")+
theme_minimal()+
...
#Loop for prepare data for SCORE PILARS COMPARISON GRAPH - CIRCULAR (PER PILARS)
```{r}
### COUNTRY Percentile DATASET ###
...
#Loop for prepare data for PERCENTIL RANKINF GRAPH - CIRCULAR (ALL PILARS)
```{r}
...
#Create labels for each column and angles (no need to adjust-with this method label are aligned automatically)
label_data3 <- data3
