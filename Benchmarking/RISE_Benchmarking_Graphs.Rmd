...
# Set Directories
```{r}
dir_int<-"./Integrated_data/" #Folder with data generated in code RISE_Benchmarking_Data
dir_out<-"./Output/"
...
#Country
country="TUN"  #use one country for tests
...
filetype <- 'pdf' #Set preferred type of graph output


#Set text size for graphs
perctext <-1.5
pillartext <- 3
...
Here we rename the variables of the raw values and the percentile ranking
```{r}
...
ggsave(reg, file=paste0(dir_out, group,'/',i,"_barplot_per.",filetype), width=25, height=10)
...
ggsave(graph2, file=paste0(dir_out, group,'/',i,"_barplot_per_group.",filetype), width=25, height=10)
...
write.csv(income_median_country,paste0(dir_out, group,'/',i,"_tables_data.csv"), row.names = FALSE)
}
```
...
ggsave(com, file=paste0(dir_out, group,'/',i,"_percentil_global.",filetype), width=12, height=13)
...
ggsave(s, file=paste0(dir_out, group,'/',i,"_score_graph_sust.",filetype), width=10, height=10)
...
ggsave(inc, file=paste0(dir_out, group,'/',i,"_score_graph_incl.",filetype), width=10, height=10)
...
ggsave(e, file=paste0(dir_out, group,'/',i,"_score_graph_eff.",filetype), width=10, height=10)
...
ggsave(r, file=paste0(dir_out, group,'/',i,"_score_graph_res.",filetype), width=10, height=10)
...
file.remove(paste0(dir_out, group,'/',i,"_percentil.csv"))
file.remove(paste0(dir_out, group,'/',i,"_score_graphs.csv"))


```


<!-- #Principal component contribution using imputePCA -->
<!-- ```{r} -->


<!-- #country_all <- country_all[!is.na(country_all$Resilience_v2),] -->
<!-- country_all <- country_all[!is.na(country_all$Efficiency_v2),] -->
...
<!-- effi<- list("(E4) Agri. land productivity ($/ha)", "(E1) Agri. value added per worker", "(E4) Total GDP/GHG emissions", "(E2) Governance effectiveness", "(E2) Control of corruption", "(E2) Regulatory quality", "(E1) Logistic Performance Index", "(E3) Individuals using internet %", "(E3) Mobile cellular subscriptions (per 100 people)", "(E2) Rule of law", "(E3) Rapid transit to resident ratio (km/million)", "(E4) *Energy intensity (MJ/$)", "(E1) Patents (per 100,000 people)",  "(E1) Digital Penetration Index", "(E4) Water productivity (GDP/m3)", "(E3) Shipping connectivity index") -->
...
<!-- sust<-list("(S1) *PM2.5 mean annual exposure (\U00B5g/m3)", "(S4) *Forest loss %", "(S3) Renewable energy consumption %","(S2) Wastewater treated (primary) %","(S2) Solid waste properly disposed %","(S2) *Municipal solid waste (kg/pc/day)","(S4) Biodiversity Habitat Index","(S4) Fisheries Index", "(S1) *GHG emissions per capita (% change)", "(S1) *CO2 emissions (MT pc)","(S3) *Ecological Footprint","(S3) *Water withdrawal per capita (m3/year pc)","(S4) *Chlorophyll-a deviations in coastal water %","(S1) *Mortality due to air pollution (per 100,000)","(S2) *Mortality due to unsafe WASH services (per 100,000)", "(S3) Water Quality SDG 6.3") -->
...
<!-- resi<-list("(R3) *Global Hunger Index", "(R3) *Deaths from conflict and terrorism (per 100,000)", "(R1) *Population affected by disasters %","(R3) *Proportion of urban population living in slums", "(R1) *Rainfall shock exposure %", "(R3) *Population exposed to epidemics (per 100,000)", "(R2) *Projected change of deaths from climate change induced diseases", "(R2) *Projected change of warm periods", "(R2) *Projected change of flood hazard", "(R4) *Dependency on external resource for health services", "(R4) *Food import dependency", "(R2) *Projected change of sea level rise impacts", "(R4) *Dependency on imported energy", "(R1) *IDPs due to natural disasters (cases per 100,000)", "(R1) *Water Stress SDG 6.4", "(R4) Physicians (per 1,000 people)") -->
