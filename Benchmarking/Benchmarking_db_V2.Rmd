...
ind_17<-c('SH.STA.SMSS.ZS', 'SH.H2O.SMDW.ZS', 'HD.HCI.OVRL','EN.ATM.PM25.MC.M3', 'NY.ADJ.SVNG.GN.ZS','SH.UHC.SRVS.CV.XD', 'ER.H2O.INTR.PC') 
...
energ_int<- energ_int%>%
  ...
  colnames(energ_int)<-c("iso3", "energ_int")  
...
#Use Total water withdrawal indicator from http://www.fao.org/nr/water/aquastat/data/query/results.html (download data as csv flat file - countries in y axis and values in x axis) - Take most recent year with data since 2010, and used GNI information of the same year. GNI data was already extracted from the WDI
...
## FAO Aquastat water withdrawals Total water withdrawal per capita m³ per person per year also taken from, downloaded from Ourworldindata

fao_withdraw<-read.csv(paste0(dir_rawv2, "water-withdrawals-per-capita.csv"))
fao_withdraw <- fao_withdraw[fao_withdraw$Year>2004,]
fao_withdraw<- fao_withdraw%>%
  group_by(Code)%>%
  dplyr::summarize(mean(Total.water.withdrawal.per.capita, na.rm = TRUE))
colnames(fao_withdraw)<-c("iso3","fao_withdraw")
...
##LMICS## 
integrated_lmics_score <- scores_inc %>% filter(income=='Lower middle income')
# Save 
write.csv(integrated_lmics_score, paste0(dir_int, "DB_SD_lmics_score.csv"), row.names = FALSE)
