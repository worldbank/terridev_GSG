...
```
#Save list with all variables (percentage and raw values that we will later include in the formula)
```{r}
...
##v0 
Refers to a set of indicators selected and vetted by the Chief Economist Office -Sustainable Development (work lead by Jason Daniel Russ). This version is updated with the criterio from November 30, 2020
##v1
...
write.csv(integrated3, paste0(dir_int, "World_SD_values_V5.csv"), row.names = FALSE)
```
